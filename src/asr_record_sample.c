...
char *getContent(char *elementstart,char *elementend,char *filepath,char *ret){	
	...
	printf("%s  ,%s  filepath:%s\n",elementstart,elementend,filepath);
	...
	//对于xmlChar* xch=BAD_CAST("book")，char* ch=(char *)(xch)
		if (!xmlStrcmp(pcur->name, BAD_CAST(elementstart)))
		{
			nptr=pcur->xmlChildrenNode;
			while (pcur != NULL)
			{
				if (!xmlStrcmp(nptr->name, BAD_CAST(elementend)))
				{
	...
	xmlCleanupParser ();
	xmlMemoryDump ();
	strcpy(ret,((char*)XML_GET_CONTENT(nptr->xmlChildrenNode)));
	return ret;
}
...
int update_lexicon(UserData *udata)
{
		...
		snprintf(update_lex_params, MAX_PARAMS_LEN - 1, 
			...
			asr_res_path = %s, sample_rate = %d, \
			grm_build_path = %s, grammar_list = %s, ",ASR_RES_PATH,SAMPLE_RATE_16K,GRM_BUILD_PATH,udata->grammar_id);
...
{
	int fd = open("./result.xml",O_RDWR|O_TRUNC);
	if (result) {
		...
		printf("******^^^&&&********** %d\n",strlen(g_result));
		write(fd,g_result,strlen(g_result));
...
static void demo_file(const char* audio_file, const char* session_begin_params)
{
	...
	struct speech_rec_notifier recnotifier = {on_result,on_speech_begin,on_speech_end};
...
void *substring1(char *ret,const char *str,int start){
	char *p = ret;
	if (str != NULL && start > 0 && strlen(str) > start)
	{
		while ( (*p = *(str + start)) != '<')
		{
		    p++,start++;
...
{
	char c,str1[1000] = {NULL};
	char retstr[100] = {NULL};
	int ret = 0 ;
	ret = run_asr(&asr_data);
	if (MSP_SUCCESS != ret) {
	...
	sprintf(str1,"speak over******************,%s\n",g_result);
	printf("str1=%s\n",str1);
	write_log(str1);
	substring1(retstr,g_result,98);
	//retstr = getContent("./result.xml","nlp","rawtext",retstr);
	bzero(str1,sizeof(str1));
	sprintf(str1,"in asr_record.c ret str:%s\n",retstr);
	printf("str1=%s\n",str1);
	write_log(str1);
	strcpy(str,retstr);
	//MSPLogout();
