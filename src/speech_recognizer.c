...
int sr_stop_listening(struct speech_rec *sr)
{
	...
	if (sr->state < SR_STATE_STARTED) {
		...
		printf("Not started or already stopped.\n");
		//write_log(pFile,str1);
		return 0;
	}
	...
	if (ret != 0) {
		...
		printf("write LAST_SAMPLE failed: %d\n", ret);
		//write_log(pFile,str1);
		QISRSessionEnd(sr->session_id, "write err");
		return ret;
	}
	sr->rec_stat = 2;
	printf("in speeacher.c2\n");
	while (sr->rec_stat != MSP_REC_STATUS_COMPLETE) {
